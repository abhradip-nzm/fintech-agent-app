...
import { readAgents } from '../../utils/storage';
import './HomePage.css';

// ─── Agently Logo (rounded-square app icon · rising trend path · 3 nodes) ─────
...
// ─── Agent Card ───────────────────────────────────────────────────────────────
const AgentCard = ({ agent, onClick, isDark }) => {
    ...
    >
      ...
      <div className="hp-card-header">
        <div style={{
          ...
          background: `linear-gradient(135deg, ${ac.p}${isDark ? '40' : '18'}, ${ac.p}${isDark ? '20' : '0c'})`,
          border: `1px solid ${ac.p}${isDark ? '55' : '30'}`,
          display: 'flex', alignItems: 'center', justifyContent: 'center', color: ac.p
        }}>
          {icon}
        </div>
...
const HomePage = () => {
  ...
  // Sync theme ref for canvas loop
  ...
  const gridCols =
    agents.length === 0 ? '1fr' :
    agents.length === 1 ? 'minmax(0, 560px)' :
    agents.length === 2 ? '1fr 1fr' :
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <button
            ...
            style={{
              background: T.adminBtnBg,
              border: T.adminBtnBorder,
          ...
          {/* Status */}
          <div
            className="hp-header-right"
            style={{
              background: T.statusBg,
              border: T.statusBorder,
            }}
          >
      ...
      <section className="hp-hero">
        <div className="hp-hero-badge">
          <span
            ...
            style={{ background: T.chipBg, border: T.chipBorder, color: T.chipColor }}
          >
        ...
        >
          AI Agent<br />
          <span style={{
            background: 'linear-gradient(135deg, #4f7cf7, #7c3aed)',
        ...
        <div
          className="hp-stats"
          style={{ background: T.statsBg, border: T.statsBorder, boxShadow: isDark ? 'none' : '0 4px 20px rgba(0,0,0,0.06)' }}
        >
          {[
            ...
            <div
              key={s.label}
              className="hp-stat"
              style={{ borderRight: i < 3 ? `1px solid ${T.statDivider}` : 'none' }}
            >
              <span className="hp-stat-value" style={{ color: T.statValueColor }}>{s.value}</span>
              <span className="hp-stat-label"  style={{ color: T.statLabelColor }}>{s.label}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="hp-cards" style={{ gridTemplateColumns: gridCols }}>
        {loading ? (
          ...
          <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: '48px 24px' }}>
            ...
            <button
              onClick={() => navigate('/admin')}
              style={{ padding: '10px 24px', borderRadius: 10, background: 'linear-gradient(135deg,#4f7cf7,#7c3aed)', color: '#fff', fontSize: 13, fontWeight: 700, border: 'none', cursor: 'pointer', fontFamily: 'Plus Jakarta Sans, sans-serif', boxShadow: '0 4px 16px rgba(79,124,247,0.35)' }}
            >
              Open Super Admin →
            </button>
          </div>
        ) : agents.map(agent => (
          <AgentCard key={agent.id} agent={agent} onClick={() => handleCardClick(agent)} isDark={isDark} />
        ))}
      </section>
